...
class moodle_gradereport_quizanalytics_external extends external_api {
	...
	public static function quizanalytics_analytic($quizid, $user_id) {
		...
		// Get information about quiz using quizid
		$quiz = $DB->get_record('quiz', array('id' => $quizid));

		// Get information about quiz attempts for this quiz
		...
		// Get total number of question present in this quiz
		...
		$questionpercategoryopt = array(
			...
			'title' => array(
				'display' => true,
				'position' => 'bottom',
				'text' => get_string('questionspercategory', 'gradereport_quizanalytics'),
			),
		);
		/* allusers */
		...
		$allusersopt = array(
			...
			'title' => array(
				'display' => false,
				'position' => 'bottom',
				'text' => get_string('hardcatalluser', 'gradereport_quizanalytics'),
			),
		);
		/* loggedinuser */
		arsort($userhardness);
		...
		$loggedinuseropt = array(
			...
			'title' => array(
				'display' => false,
				'position' => 'bottom',
				'text' => get_string('hardcatlogginuser', 'gradereport_quizanalytics'),
			),
		);
		/* lastattemptsummary */
		...
		if (!empty($usersgradedattempts)) {
			...
			$snapshotopt[1] = array(
				'title' => array(
					'display' => true,
					'position' => 'bottom',
					'text' => 'Attempts Snapshot( timetaken: 0min )',
				),
			);
		}
		/* timechart */
		...
		$gradeanalysisopt = array(
			'title' => array(
				'display' => true,
				'text' => get_string('noofstudents', 'gradereport_quizanalytics'),
				'position' => 'bottom',
			),
		...
		foreach ($totalquestions as $totalquestion) {
			...
			$usercorrectresponse = $DB->get_record_sql(
				...
				array($sequencenumber, $quizid, $totalquestion->id, $user_id)
			);
		...
		$quesanalysisdata = array(
			...
			'datasets' => array(
				...
				array(
					...
					'borderColor' => "#3cba9f",
					'fill' => false,
					'label' => get_string('partialcorrect', 'gradereport_quizanalytics'),
				),
				array(
					'data' => $userunattempted,
					'borderColor' => "#c45850",
					'fill' => false,
