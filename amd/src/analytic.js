define(['jquery', 'core/ajax', 'core/str', 'gradereport_quizanalytics/datatables'], function ($, ajax, str, dt) {
    return {
        ...
        analytic: function () {
            ...
            // Add event to all #userSelect element and style it's viewanalytic link 
            userSelects.forEach((userSelect) => {
                ...
                if (viewAnalyticsLink && userSelect) {
                    ...
                    userSelect.addEventListener("change", function () {
                        // Set the viewanalytic link un-clickable value of userselect is -1
                        // Otherwise viewanlytic link will be clickable for user
                        if (userSelect.value === '-1') {
                            viewAnalyticsLink.style.pointerEvents = 'none';
                            viewAnalyticsLink.style.color = '#999';
                        }
            ...
            // Add click event to viewanalytic link
            viewAnalyticsLinks.forEach(function (viewAnalyticsLink) {
                if (viewAnalyticsLink) {
                    viewAnalyticsLink.addEventListener("click", function () {
                        // collect quizId and userID
                        const userSelect = viewAnalyticsLink.parentNode.parentNode.querySelector("#userSelect");
                        let quizId = viewAnalyticsLink.getAttribute('data-quiz_id');
                        userID = userSelect ? userSelect.value : -1;

                        // Ajax call for collect usefull information about the user related to the quiz
                        ...
                        // Afte the ajax call use this information
