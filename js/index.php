...
///////////////////////////////////////////////////////////////////////////
// Edit page and parepage base on filter...
if (isset($_POST["show-all"]) && $_POST["show-all"] === 'on') {
    $perpage = count($DB->get_records('quiz', array('course' => $courseid)));
    redirect(new moodle_url($CFG->wwwroot . '/grade/report/quizanalytics/index.php', array('id' => $courseid, 'perpage' => $perpage, 'page' => 0)));
}

if (isset($_POST["row-number"]) && !empty(trim($_POST["row-number"]))) {
    $page = 0;
    $perpage = $_POST["row-number"];
    redirect(new moodle_url($CFG->wwwroot . '/grade/report/quizanalytics/index.php', array('id' => $courseid, 'perpage' => $perpage, 'page' => 0)));
}
////////////////////////////////////////////////////////////////////////////
...
//////////////////////////////////////////////////////////////////////////
// This is the filter part in bottom of the heading.....
echo "<div id='quizanalytics-filter'>
        <form method='post'>
            <div>
            ...
            <div>
                ...
                <input type='number' id='row-number' name='row-number' min='1' stape='1'>
            </div>
            <span>|</span>
...
if (!$getquiz) {
    ...
    foreach ($getquiz as $getquizkey => $getquizval) {
        ...
        if (!$is_student) {
            $getquizattempts = $DB->get_records(
                'quiz_attempts',
                array(
                    'quiz' => $getquizval->id,
                    'state' => 'finished'
        ...
        if (!$is_student) {
            ...
            $select = "<select id='userSelect'><option value='-1' selected>" . get_string('user_select', 'gradereport_quizanalytics') . "</option>";
